...
void measerement_for::on_firstTypeMesageBut_clicked()
{
    ...
    if (!senderThread) {
            senderThread = new MessageSendThreadMEAS(this);
            connect(senderThread, &MessageSendThreadMEAS::messageSend, this, &measerement_for::handleMessageSend);
    }
            senderThread->message =" MEAS 1 " + ui->firstTypeMsgText->toPlainText().toStdString();
            senderThread->start();
}
...
//    }
    if (!receiverThread) {
            receiverThread = new MessageReceiverThreadMEAS(this);
            connect(receiverThread, &MessageReceiverThreadMEAS::messageReceived, this, &measerement_for::handleMessageReceived);
    }
...
MessageReceiverThreadMEAS::MessageReceiverThreadMEAS(QObject *parent)
    : QThread(parent)
{
}

void MessageReceiverThreadMEAS::run()
{
    qDebug()<<"HELLO";
...
void measerement_for::handleMessageReceived(const QString& message)
{
...
MessageSendThreadMEAS::MessageSendThreadMEAS(QObject *parent)
    : QThread(parent)
{
}

void MessageSendThreadMEAS::run()
{
    int sock = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
    if (sock < 0) {
        std::cerr << "Ошибка создания сокета" << std::endl;
...
void measerement_for::handleMessageSend()
{
