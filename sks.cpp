...
void sks::on_firstTypeMsgBut_clicked()
{
    ...
    if (!senderThread) {
            senderThread = new MessageSendThreadSKS(this);
            connect(senderThread, &MessageSendThreadSKS::messageSend, this, &sks::handleMessageSend);
    }
            senderThread->message =" SKS 1 " + ui->firstTypeMsgText->toPlainText().toStdString();
            senderThread->start();
}
...
//    }
    if (!receiverThread) {
            receiverThread = new MessageReceiverThreadSKS(this);
            connect(receiverThread, &MessageReceiverThreadSKS::messageReceived, this, &sks::handleMessageReceived);
    }
            receiverThread->start();
}


MessageReceiverThreadSKS::MessageReceiverThreadSKS(QObject *parent)
    : QThread(parent)
{
}

void MessageReceiverThreadSKS::run()
{
    qDebug()<<"HELLO";
...
void sks::handleMessageReceived(const QString& message)
{
...
MessageSendThreadSKS::MessageSendThreadSKS(QObject *parent)
    : QThread(parent)
{
}

void MessageSendThreadSKS::run()
{
    int sock = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
    if (sock < 0) {
        std::cerr << "Ошибка создания сокета" << std::endl;
