...
void dist_system::on_firstTypeMesageBut_clicked()
{
    ...
    if (!senderThread) {
            senderThread = new MessageSendThreadDIST(this);
            connect(senderThread, &MessageSendThreadDIST::messageSend, this, &dist_system::handleMessageSend);
    }
            senderThread->message =" DIST 1 " + ui->firstTypeMsgText->toPlainText().toStdString();
            senderThread->start();
}
...
//    }
    if (!receiverThread) {
            receiverThread = new MessageReceiverThreadDIST(this);
            connect(receiverThread, &MessageReceiverThreadDIST::messageReceived, this, &dist_system::handleMessageReceived);
    }
...
MessageReceiverThreadDIST::MessageReceiverThreadDIST(QObject *parent)
    : QThread(parent)
{
}

void MessageReceiverThreadDIST::run()
{
    qDebug()<<"HELLO";
...
void dist_system::handleMessageReceived(const QString& message)
{
...
MessageSendThreadDIST::MessageSendThreadDIST(QObject *parent)
    : QThread(parent)
{
}

void MessageSendThreadDIST::run()
{
    int sock = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
    if (sock < 0) {
        std::cerr << "Ошибка создания сокета" << std::endl;
...
void dist_system::handleMessageSend()
{
