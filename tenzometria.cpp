...
void tenzometria::on_firstTypeMesageBut_clicked()
{
    ...
    if (!senderThread) {
            senderThread = new MessageSendThreadTenz(this);
            connect(senderThread, &MessageSendThreadTenz::messageSend, this, &tenzometria::handleMessageSend);
    }
...
MessageReceiverThreadTenz::MessageReceiverThreadTenz(QObject *parent)
    : QThread(parent)
{
}

void MessageReceiverThreadTenz::run()
{
    qDebug()<<"HELLO";
...
void tenzometria::handleMessageReceived(const QString& message)
{
    ui->textEdit_3->insertPlainText(message);
}

MessageSendThreadTenz::MessageSendThreadTenz(QObject *parent)
    : QThread(parent)
{
}

void MessageSendThreadTenz::run()
{
    int sock = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
    if (sock < 0) {
        std::cerr << "Ошибка создания сокета" << std::endl;
...
void tenzometria::handleMessageSend()
{
